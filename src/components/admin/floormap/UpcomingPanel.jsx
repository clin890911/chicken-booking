...
function BookingCard({ b, now, kind, onClickBooking, onAssignTable, onMoveTable, onSeat, onNoshow, onComplete, perms }) {
  ...
  // 併桌訂位（有額外桌）move 只換主桌會留孤兒桌 → 不給改桌（維持唯讀徽章）。
  const canMove = assigned && !!onMoveTable && perms.booking && perms.table && !(b.extraTableIds || []).length
        ...
        <div className="space-y-2">
          ...
          {overdue.map(b => (
            <BookingCard key={b.id} b={b} now={now} kind={kindOf(b)}
              onClickBooking={onClickBooking} onAssignTable={onAssignTable} onMoveTable={onMoveTable} onSeat={handleSeat} onNoshow={handleNoshow}
