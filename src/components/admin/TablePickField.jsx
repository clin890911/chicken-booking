...
// 這裡改成時段下方看得見的晶片：候選＝現在空桌、今日可用、容量夠、且依所選時段不撞別筆預配/團保的單桌
// （排序沿用 findSuitableTables：浪費小 → 1F 優先 → 桌號）。第一張預選並標「建議」（多數店員不用改），
...
export default function TablePickField({ hasSlot, candidates = [], choice, onPick, notice, emptyReason }) {
      ...
      <div className="space-y-2">
        ...
        {candidates.length > COLLAPSE_AT && (
          <button type="button" onClick={() => setExpanded(e => !e)}
            ...
            {expanded ? '收合 ▴' : `＋ 其他 ${hiddenCount} 張空桌 ▾`}
          </button>
        )}
